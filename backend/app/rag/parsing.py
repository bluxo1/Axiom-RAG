...
def _resolve_public_addresses(host: str, port: int) -> tuple[str, ...]:
    """Resolve a host once and return only if every answer is globally routable."""
    try:
    ...
    for info in infos:
        ...
        address_text = (
            raw_address.split("%", maxsplit=1)[0] if isinstance(raw_address, str) else raw_address
        )
...
def ensure_safe_url(url: str) -> str:
    ...
    _, host, port = _parse_safe_url(url)
...
def _fetch_safe_url(url: str) -> tuple[str, bytes]:
    ...
    for redirect_count in range(_URL_FETCH_MAX_REDIRECTS + 1):
        _, host, port = _parse_safe_url(current_url)
