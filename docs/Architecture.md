...
5. **Confidence** — hybrid score decides: answer as-is | flag | fall back (runs only after grounding passes)
...
flowchart TD
    ...
    subgraph Query Path
        ...
        GEN -- INSUFFICIENT_EVIDENCE --> SRCH
...
- **Chunker:** recursive character splitting, target chunk size 512 tokens with 64-token overlap between consecutive chunks; each chunk gets a stable `chunk_id` (hash of doc_id + index)
...
Grounding is a **hard gate**: it runs *before* the confidence router on every path (high-confidence, flagged, and web-fallback). No unverified answer ever reaches the router or the user.
...
Claims with no valid citation are **stripped from the answer** (not silently kept); a stripped answer drops confidence via the coverage term. If stripping would leave no answer, the pipeline returns the honest refusal card.

Failures → answer regenerated (max 1 retry) or flagged.

After verification passes, the verifier rewrites surviving citations to **display ids** `c1, c2, ...` and replaces markers in the answer text (see Design §3.3).
...
- `< 0.45` → fallback
- LLM returns `INSUFFICIENT_EVIDENCE` or zero retrieved hits → fallback

All router decisions (including flagged and refusal outcomes) are logged with their inputs.
...
- Results injected as additional context with `source=web` and live URLs; same grounding rules apply — web answers re-enter generation → grounding → confidence routing
...
4. Verify citations (hard gate) → compute confidence
5. Route: respond / flag / fallback → persist everything to Postgres
6. Stream to frontend: `status` event immediately ("grounding..."), then the verified answer token-by-token with citation cards. Answer tokens only stream *after* verification passes; if verification fails and triggers a retry, the retry completes before any answer token is sent.
